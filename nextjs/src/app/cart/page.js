      ...
      {!items.length ? (
        ...
        <section className="cart-shell">
          <div className="cart-layout cart-layout--page">
                    ...
                    <article key={lineKey} className="cart-row">
                      <Link href={productId ? `/products/${productId}` : "/products"} className="cart-row__product">
                        ...
                        <div className="cart-row__meta">
                          <span className="cart-row__category">{formatCategoryLabel(item.category || item.categoryName || "Product")}</span>
                          <h3>{item.name}</h3>
                          {upgradeLabel ? <small>{upgradeLabel}</small> : null}
                          <p className="cart-row__price-mobile">{formatCurrency(pricing.currentUnitPrice)}</p>
                        </div>
                      </Link>
                      <div className="cart-row__qty">
                        <button type="button" onClick={() => updateQuantity(productId, Number(item.qty || 1) - 1, lineKey)} aria-label={`Decrease ${item.name} quantity`}>
                          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
                        </button>
                        <span className="cart-row__qty-value">{item.qty}</span>
                      ...
                      <div className="cart-row__subtotal-block">
                        ...
                        <p className="cart-row__total-label-mobile">Total</p>
                      </div>
                      <button
                        type="button"
            ...
            <aside className="cart-summary panel">
              ...
              <div className="cart-summary__lines">
                ...
                <div className="cart-summary__line">
                  <span>Shipping</span>
                  <strong className={shipping === 0 ? "is-free" : ""}>{shipping === 0 ? "Free" : formatCurrency(shipping)}</strong>
                </div>
                <div className="cart-summary__line">
                  <span>Tax (VAT)</span>
              ...
              <div className="cart-summary__actions">
                <Link href="/checkout" className="cart-summary__checkout">
                  Proceed to Checkout
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </Link>
                <Link href="/products" className="cart-summary__continue">Continue Shopping</Link>
              </div>
            </aside>
          </div>
          ...
          <div className="cart-trust-badge">
            ...
            <p>Secure checkout powered by DEETECH Ghana. Your data is encrypted and protected.</p>
          </div>
